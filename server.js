...
// IMPORTANT: Defined before routes
...
app.get('/api/admin/users', authMiddleware, (req, res) => {
  ...
  const users = db.prepare('SELECT id, username, email, role, joined, banned FROM users ORDER BY id DESC').all();
  res.json(users);
});
...
app.get('/api/stats', (req, res) => {
  ...
  res.json({ users: count, battles: 0, rappers: 0 });
});
